...
# Tool: Access Hole Cover for Z-Bridge      #
# PLM:  DOC-0009-1243                       #
#############################################
...
#   - Requires python 3.6
#   - Linear amplifiers to reduce EMI: Aerotech Ensemble ML
...
# Program Variables
stateArray = [0, 0, 0, 0, 0]  # Scan Circ, Scan Radial, Position Vert, N/A, Performing Scan
...
def initScan():
    ...
    if (stateArray[4] == 0):
        ...
        if (len(eScanWindow_scanStart.get()) == 0 or len(eScanWindow_scanStop.get()) == 0 or len(
                eScanWindow_indexStart.get()) == 0 or len(eScanWindow_indexStop.get()) == 0 or len(
                eScanWindow_indexSize.get()) == 0):
        ...
        # Disable Position Vert inputs
        bPos_ENABLE_vert.config(state="disabled")
        bPos_UP.config(state="disabled")
        bPos_DOWN.config(state="disabled")
        bPos_Set_vert.config(state="disabled")
        bPos_GoTo_vert.config(state="disabled")
        bPos_Increment_vert.config(state="disabled")
        sPos_vert_velocity.config(state="disabled")
        ePos_vert_set.config(state="disabled")
        ePos_vert_goto.config(state="disabled")
        ePos_vert_increment.config(state="disabled")
        ...
        if (v.get() == 0):
            ...
            while (TRUE):
                ...
                if ((indexStop - indexStart) % indexSize == 0 and indexStop == rVar and scanArray[scanIndex - 2][
                    0] == rVar):
                    break
                # Bi-directional scan
                if (d.get() == 0):
                    if ((
                            indexStop - indexStart) % indexSize != 0 and rVar + indexSize > indexStop and scanStart == thetaVar):
                        break
                    if ((scanIndex + 1) % 2 == 1):
                        # Increment rVar
                        rVar = rVar + indexSize
                ...
                # Uni-directional scan
                    ...
                    if ((
                            indexStop - indexStart) % indexSize != 0 and rVar + indexSize > indexStop and scanStop == thetaVar):
                        break
                    if ((scanIndex + 3) % 3 == 0):
                        # Increment rVar
                        rVar = rVar + indexSize
        ...
        # Radial Exam:
            ...
            while (TRUE):
                # scanArray complete where delta index is divisible by indexSize
                if ((indexStop - indexStart) % indexSize == 0 and scanArray[scanIndex - 1][2] == abs(
                        scanStop - scanStart) and thetaVar == indexStop):
                    break
                # Bi-directional scan
                if (d.get() == 0):
                    # condition if Bi-directional and one more scan line to center is necessary
                    if ((indexStop - indexStart) % indexSize != 0 and rVar == scanStart and (
                            thetaVar + indexSize) > indexStop):
                        break
                    if (scanIndex % 2 == 1):
                        # Toggle rVar
                ...
                # Uni-directional scan
                    ...
                    if ((indexStop - indexStart) % indexSize != 0 and rVar == scanStop and (
                            thetaVar + indexSize) > indexStop):
                        break
                    if ((scanIndex + 3) % 3 == 1):
                        # Toggle rVar
...
def stopScan():
    ...
    # Enable Position Vert inputs
    bPos_ENABLE_vert.config(state="normal")
    bPos_UP.config(state="normal")
    bPos_DOWN.config(state="normal")
    bPos_Set_vert.config(state="normal")
    bPos_GoTo_vert.config(state="normal")
    bPos_Increment_vert.config(state="normal")
    sPos_vert_velocity.config(state="normal")
    ePos_vert_set.config(state="normal")
    ePos_vert_goto.config(state="normal")
    ePos_vert_increment.config(state="normal")
...
def disableAxis(axis):
    if (axis == 0):
        ...
        if ((0b1 & int(acmd(ser, "AXISSTATUS(" + axisNameArray[axis] + ")"))) == 0):
            ...
            bPos_ENABLE_vert.config(text="OFF", bg="red")
            bPos_UP.config(state="disabled")
            bPos_DOWN.config(state="disabled")
            bPos_Set_vert.config(state="disabled")
            bPos_GoTo_vert.config(state="disabled")
...
def enableAxisBtn(axis):
    if (axis == 0):
        ...
        if (stateArray[axis] == 0):
            acmd(ser, "ENABLE " + axisNameArray[axis])
            if ((0b1 & int(acmd(ser, "AXISSTATUS(" + axisNameArray[axis] + ")"))) == 1):
                bPos_ENABLE_vert.config(text="ON", bg="#00aa00")
                bPos_UP.config(state="active")
                bPos_DOWN.config(state="active")
                bPos_Set_vert.config(state="active")
                bPos_GoTo_vert.config(state="active")
                bPos_Increment_vert.config(state="active")
                stateArray[axis] = 1
        else:
            acmd(ser, "DISABLE " + axisNameArray[axis])
            if ((0b1 & int(acmd(ser, "AXISSTATUS(" + axisNameArray[axis] + ")"))) == 0):
...
def moveInc(axis):
    if (axis == 0):
        ...
        distance = str(ePos_vert_increment.get())
        speed = str(sPos_vert_velocity.get())
...
def moveTo(axis):
    if (axis == 0):
        ...
        distance = str(ePos_vert_goto.get())
        if (distance == ""):
            return
        speed = str(sPos_vert_velocity.get())
...
def startJog(axis, direction):
    ...
    elif (axis == 2 and stateArray[2] and bPos_DOWN['state'] != 'disabled' and bPos_UP['state'] != 'disabled'):
        acmd(ser, "ABORT Drive_Axis_C")
        if (direction == "forward"):
            speed = str(sPos_vert_velocity.get())
        elif (direction == "backward"):
            speed = str(-1 * sPos_vert_velocity.get())
...
def setPos(axis):
    if (axis == 0):
        ...
        posToSet = str(ePos_vert_set.get())
...
def updateFBK():
    ...
    pos = round(float(pos), 2)
    pos = format(pos, '.2f')
    posVertPosition.set(pos)

    cur = acmd(ser, "IFBK(Drive_Axis_C)")
    cur = float(cur) * 1000
    cur = round(cur)
    posVertCurrent.set(cur)
        ...
        if (fault_C != 0):
            ...
            for i in range(0, len(faultArray)):
                if ((fault_C & (faultMask << i)) != 0):
                    messagebox.showinfo("FAULT: Position Vert Axis", "%s" % faultArray[i])
            acmd(ser, "ACKNOWLEDGEALL")
...
def scanTimeUpdate():
    if (stateArray[4] == 1):
        ...
        # When scanIndex is greater than 0 but not larger than array so divide by zero never happens
            ...
            if (abs(float(eScan_circ_position.get()) - scanArray[scanIndex][1]) < ERROR and abs(
                    float(eScan_rad_position.get()) - scanArray[scanIndex][0]) > ERROR):
            ...
            elif (abs(float(eScan_rad_position.get()) - scanArray[scanIndex][0]) < ERROR and abs(
                    float(eScan_circ_position.get()) - scanArray[scanIndex][1]) > ERROR):
...
def nextScanMove():
    ...
    if (stateArray[4] == 1):
        ...
        if (abs(float(pos_A) - scanArray[scanIndex][1]) < ERROR and abs(
                float(pos_B) - scanArray[scanIndex][0]) < ERROR):
            # Increment scanIndex
            scanIndex += 1

            # Convert in/sec to deg/sec
            convertedSpeed = (abs(scanArray[scanIndex - 1][1] - scanArray[scanIndex][1]) / (
                        scanArray[scanIndex][2] / scanSpeed))
...
def controllerReset(ser):
    ...
    result = messagebox.askquestion("RESET?",
                                    "ALL position data will be lost and GUI will shutdown.\nAre you sure you want to reset controller?",
                                    icon='warning')
    if result == 'yes':
        acmd(ser, "RESET")
    else:
...
root.title("Tooling Inspection Motion Controller - Access Hole Cover, Z-Bridge")

scanCircPosition = StringVar()
scanCircCurrent = StringVar()
scanRadPosition = StringVar()
scanRadCurrent = StringVar()
posVertPosition = StringVar()
posVertCurrent = StringVar()
...
bScan_CCW = Button(scan_frame, text="CCW", activeforeground="black", activebackground="#00aa00", bg="#00aa00", width=10,
                   state=DISABLED)
bScan_CW = Button(scan_frame, text="CW", activeforeground="black", activebackground="#00aa00", bg="#00aa00", width=10,
                  state=DISABLED)
bScan_ENABLE_circ = Button(scan_frame, text="OFF", fg="black", bg="red", height=2, width=6, padx=3, pady=3,
                           command=lambda: enableAxisBtn(0))
bScan_Set_circ = Button(scan_frame, text=" Set ", activeforeground="black", activebackground="#00aa00", bg="#00aa00",
                        state=DISABLED, command=lambda: setPos(0))
bScan_GoTo_circ = Button(scan_frame, text="GoTo", activeforeground="black", activebackground="#00aa00", bg="#00aa00",
                         state=DISABLED, command=lambda: moveTo(0))
bScan_Increment_circ = Button(scan_frame, text="Index", activeforeground="black", activebackground="#00aa00",
                              bg="#00aa00", state=DISABLED, command=lambda: moveInc(0))
...
bScan_Set_rad = Button(scan_frame, text=" Set ", activeforeground="black", activebackground="#00aa00", bg="#00aa00",
                       state=DISABLED, command=lambda: setPos(1))
bScan_GoTo_rad = Button(scan_frame, text="GoTo", activeforeground="black", activebackground="#00aa00", bg="#00aa00",
                        state=DISABLED, command=lambda: moveTo(1))
bScan_Increment_rad = Button(scan_frame, text="Index", activeforeground="black", activebackground="#00aa00",
                             bg="#00aa00", state=DISABLED, command=lambda: moveInc(1))
...
bPos_UP = Button(position_frame, text="UP", activeforeground="black", activebackground="#00aa00", bg="#00aa00",
                 width=10, state=DISABLED, command=lambda: moveInc(2))
bPos_DOWN = Button(position_frame, text="DOWN", activeforeground="black", activebackground="#00aa00", bg="#00aa00",
                   width=10, state=DISABLED, command=lambda: moveInc(2))
bPos_ENABLE_vert = Button(position_frame, text="OFF", fg="black", bg="red", height=2, width=6, padx=3, pady=3,
                          command=lambda: enableAxisBtn(2))
bPos_Set_vert = Button(position_frame, text=" Set ", activeforeground="black", activebackground="#00aa00", bg="#00aa00",
                       state=DISABLED, command=lambda: setPos(2))
bPos_GoTo_vert = Button(position_frame, text="GoTo", activeforeground="black", activebackground="#00aa00", bg="#00aa00",
                        state=DISABLED, command=lambda: moveTo(2))
bPos_Increment_vert = Button(position_frame, text="Index", activeforeground="black", activebackground="#00aa00",
                             bg="#00aa00", state=DISABLED, command=lambda: moveInc(2))
ePos_vert_position = Entry(position_frame, state="readonly", width=10, textvariable=posVertPosition)
ePos_vert_current = Entry(position_frame, state="readonly", width=10, textvariable=posVertCurrent)
...
lPos10 = Label(position_frame, text="Vertical", height=2, font=("Helvetica", 14))
...
bPos_ENABLE_vert.grid(row=1, column=0, rowspan=3, padx=15)
bPos_UP.grid(row=1, column=1, rowspan=2, padx=3)
bPos_DOWN.grid(row=1, column=2, rowspan=2, padx=3)
lPos1.grid(row=0, column=3, columnspan=2, sticky=S)
lPos2.grid(row=0, column=4, columnspan=2, sticky=S)
ePos_vert_position.grid(row=1, column=3, columnspan=2, padx=3)
ePos_vert_current.grid(row=1, column=4, columnspan=2)

ePos_vert_set.grid(row=3, column=3, padx=2)
ePos_vert_goto.grid(row=3, column=4, padx=2)
ePos_vert_increment.grid(row=3, column=5, padx=2)
bPos_Set_vert.grid(row=4, column=3, pady=5)
bPos_GoTo_vert.grid(row=4, column=4, pady=5)
bPos_Increment_vert.grid(row=4, column=5, pady=5)

sPos_vert_velocity.grid(row=3, column=1, columnspan=2, rowspan=2)
...
bScanWindow_Start = Button(scanWindow_frame, text="START", activeforeground="black", activebackground="#00aa00",
                           bg="#00aa00", width=10, command=initScan)
bScanWindow_Stop = Button(scanWindow_frame, text="STOP", activeforeground="black", activebackground="#00aa00",
                          bg="#00aa00", width=10, state=DISABLED, command=stopScan)
bScanWindow_Pause = Button(scanWindow_frame, text="PAUSE", activeforeground="black", activebackground="#00aa00",
                           bg="#00aa00", width=10, state=DISABLED, command=pauseScan)
bScanWindow_Resume = Button(scanWindow_frame, text="RESUME", activeforeground="black", activebackground="#00aa00",
                            bg="#00aa00", width=10, state=DISABLED, command=resumeScan)
eScanWindow_time = Entry(scanWindow_frame, state="readonly", width=10, textvariable=scanTimeText)
radio1 = Radiobutton(scanWindow_frame, text="     Circ                 Radial", variable=v, value=0,
                     command=changeUnits)
...
sScanWindow_velocity = Scale(scanWindow_frame, from_=0.01, to=0.5, orient=HORIZONTAL, length=150,
                             label="            Velocity in/s", resolution=0.01)
...
bLogoWindow_Reset = Button(logoWindow_frame, text="RESET", width=10, relief=RAISED,
                           command=lambda: controllerReset(ser))
...
bPos_UP.bind('<ButtonPress-1>', lambda event, axis=2, direction="backward": startJog(axis, direction))
bPos_UP.bind('<ButtonRelease-1>', lambda event, axis=2: stopJog(axis))
bPos_DOWN.bind('<ButtonPress-1>', lambda event, axis=2, direction="forward": startJog(axis, direction))
bPos_DOWN.bind('<ButtonRelease-1>', lambda event, axis=2: stopJog(axis))
